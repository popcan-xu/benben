class Paginater():
    ...
    def __init__(self, url_address, cur_page_num, total_rows, one_page_lines=10, page_maxtag=9):
        ...
        :param one_page_lines: 每页显示的记录数
        :param page_maxtag: 页面上显示的页码标签个数
        """
        self.url_address = url_address
        self.page_maxtag = page_maxtag
        """
        根据总记录数计算出总页数
        通过divmod()函数取得商和余数，有余数时，总页数为商加上1
        同时判断当前页的数值是否大于总页数
        如果大于总页数，设置当前页等于最后一页的页数
        """
        ...
        try:
            # 参数page传递的值是字符类型，需要转化为整数类型
            cur_page_num = int(cur_page_num)
            # 如果当前页码大于最大页码，设置当前页码等于最后一页的页码
        ...
        """
        如果计算出的页码标签比总页数大，设置最后的页码标签为总页数
        设置页面中页面标签开始数等于总页数减掉前面设置的页码标签数(page_maxtag)加1
        """
    ...
    def html_page(self):
        ...
        # 首页代码
        html_page.append(
            '<li><a href="{}?page=1"><font size="1" face="verdana">首页</font></a></li>'.format(self.url_address))
        # 上一页页码标签的HTML代码，如果当前是第一页，设置上一页页码标签为非可用状态
        if self.cur_page_num <= 1:
            html_page.append(
                '<li class="disabled"><a href="#"><span aria-hidden="true"><font size="1" face="verdana">&laquo;</font></span></a></li>'.format(
                    self.cur_page_num - 1))
        else:
            # 上一页页码标签的HTML代码
            html_page.append(
                '<li><a href="{}?page={}"><span aria-hidden="true"><font size="1" face="verdana">&laquo;</font></span></a></li>'.format(
                    self.url_address, self.cur_page_num - 1))
        # 依次取页码标签，注意切片函数的用法
        for i in range(self.page_start, self.page_end + 1):
            # 如果等于当前页就加一个active样式类
            if i == self.cur_page_num:
                tmp = '<li class="active"><a href="{0}?page={1}"><font size="1" face="verdana">{1}</font></a></li>'.format(
                    self.url_address, i)
            else:
                tmp = '<li><a href="{0}?page={1}"><font size="1" face="verdana">{1}</font></a></li>'.format(
                    self.url_address, i)
        ...
        if self.cur_page_num >= self.total_page:
            html_page.append(
                '<li class="disabled"><a href="#"><span aria-hidden="true"><font size="1" face="verdana">&raquo;</font></span></a></li>')
        else:
            html_page.append(
                '<li><a href="{}?page={}"><span aria-hidden="true"><font size="1" face="verdana">&raquo;</font></span></a></li>'.format(
                    self.url_address, self.cur_page_num + 1))
        # 最后一页页码标签的HTML代码
        html_page.append(
            '<li><a href="{}?page={}"><font size="1" face="verdana">尾页</font></a></li>'.format(self.url_address,
                                                                                               self.total_page))
        # 把HTML连接起来
        page_nav = "".join(html_page)
        return page_nav

    # 把data_start()方法当作属性来用
    @property
    def data_start(self):
